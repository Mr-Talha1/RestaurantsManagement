...
namespace TBAppBackend.Security
{
    public class JwtFactory : IJwtFactory
    {
        ...
        public async Task<string> LoginToken(string Role, string UserID, string MerchantId, string BranchId)
        {
            _jwtOptions.ResetTokenIssuanceDate();
            var claims = new[] {
                 new Claim(JwtRegisteredClaimNames.Sub, Role),
                  new Claim(JwtRegisteredClaimNames.UniqueName, UserID),
                  new Claim(JwtRegisteredClaimNames.GivenName, BranchId),
        ...
        public async Task<getTokenDetails> ValidateJwtToken(string token)
        {
            ...
            try
            {
                ...
                var BranchId = jwtToken.Claims.First(x => x.Type == JwtRegisteredClaimNames.GivenName).Value;
                ...
                dt.BranchId = int.Parse(BranchId);
                // return user id from JWT token if validation successful
                return dt;
            }
            catch
